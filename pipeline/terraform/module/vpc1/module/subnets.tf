...
resource "aws_subnet" "public" {
  ...
  tags = {
    Name        = "${var.vpc_name}-${var.tags.Environment}-public-subnet-${element(var.availability_zones, count.index)}"
...
resource "aws_subnet" "private" {
  ...
  tags = {
    "Name"      = "${var.vpc_name}-${var.tags.Environment}-private-subnet-${element(var.availability_zones, count.index)}"
    Owner       = var.tags.Owner
    Project     = var.tags.Project
    Environment = var.tags.Environment
    Create_By   = var.tags.Create_By
...
resource "aws_eip" "nat" {
  ...
  vpc   = true
  tags = {
    Name        = format("%s-%s-eip", var.vpc_name, var.tags.Environment)
    Owner       = var.tags.Owner
    Project     = var.tags.Project
    Environment = var.tags.Environment
    Create_By   = var.tags.Create_By
...
resource "aws_nat_gateway" "main" {
  ...
  tags = {
    Name        = "${var.vpc_name}-${var.tags.Environment}-nat-${element(var.availability_zones, count.index)}"
    Owner       = var.tags.Owner
    Project     = var.tags.Project
    Environment = var.tags.Environment
    Create_By   = var.tags.Create_By
